...
float max_element(vector<float>::iterator begin, vector<float>::iterator end){
    vector<float>::iterator tmp = begin+1;
    float rst = *begin;
    while(tmp != end){
        if(*tmp > rst)
            rst = *tmp;
...
float min_element(vector<float>::iterator begin, vector<float>::iterator end){
    vector<float>::iterator tmp = begin+1;
    float rst = *begin;
    while(tmp != end){
        if(*tmp < rst)
