...
def findtitleheight(heights,bins=20):
    ...
    #plt.plot([0]+[(m+(M-m)*(k/float(bins))) for k,x in sorted(freqs.items())],
    #         [0]+[(x) for k,x in sorted(freqs.items())])
    #plt.show()
...
''' *** Given a filename for ReadTextLines.py's readxml's output,
    ...
    *** '''
def parse(boxlist, verbose=True): #TODO: update documentation
   contents=[]; coordinates=[];
   for b in boxlist:   
      contents.append('')
      y, x, h, w = [b[s] for s in ('y', 'x', 'height', 'width')]
      coordinates.append(((y, x), (y+h, x+w)))
      heights.append(h)
   if verbose:
...
def getstrips(contents,coordinates,heights):
   ...
   for j in textcoors:
       for i in coordinates:
           if i is j: continue
           if sitson_(j,i):
               newtextcoors.append(j)
               break
       else:
           titlecoors.append(j)
   return titlecoors,newtextcoors
...
def main():
  ...
  if os.path.isfile(outname):
    exit()
  '''
  *** Step 0: Fetch Strips from Intermediate Format ***'''
  def list_from_dict(d):
      x, y, h, w = (d[k] for k in 'x y height width'.split())
      return [[y,x],[y+h, x+w]]
  with open(inname) as f: 
      my_json = eval(f.read())
      boxlist = my_json[0]['annotations']
  titlestrips,textstrips = [], []
  for a in boxlist:
     if a["class"]=="title": titlestrips.append(list_from_dict(a))
     elif a["class"]=="article": textstrips.append(list_from_dict(a))
     else:
         print('Unknown class %s in VerticalDominanceLinker!' % a["class"])
         assert(False)
  if len(titlestrips) == 0:
    os.system('cp '+inname+' '+outname)
    exit()
  DONT_MERGE_JUST_SHOW=False
  if DONT_MERGE_JUST_SHOW:
      titleblocks, articleblocks = titlestrips, [[ts] for ts in textstrips];
      title_assignments = list(range(len(titleblocks)))
      print("hello")
  ...
  *** Step 4: Write to JSON: *** '''
  anns = []
  for j,((y,x),(Y,X)) in enumerate(titleblocks):
