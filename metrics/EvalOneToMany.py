...
NUM_CLASS = 3 # the number of classes, including title, article and graphics
...
class EvalOneToMany:
	...
	def __init__(self, out_folder=None, seg_folder=None, gt_path=None, seg_path=None, img_path=None, xml_path=None, imp_name=None):
	...
	def evaluate(self, segment=None, gt=None, is_block_level=False):
		if (segment is not None):
			if is_block_level:
				self.seg_to_eval = box_from_json(segment)
				self.ground_truth = box_from_json(gt)
				print len(segment)
			else:
				self.seg_to_eval = seg_from_json(segment)
				self.ground_truth = seg_from_json(gt)
			...
			for j, g in enumerate(self.ground_truth.segs):
				for k, s in enumerate(self.seg_to_eval.segs):
					...
					if g.label != LABELS[c] or s.label != LABELS[c]:
						coef = TYPE_MISMATCH_COEFFICIENT
